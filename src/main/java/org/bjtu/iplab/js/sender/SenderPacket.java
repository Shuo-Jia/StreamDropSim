...
public class SenderPacket extends PacketInfo {
    ...
    public SenderPacket(int send_gop_num, int send_packet_num, byte[] send_sps_nalu_bytes, byte[] send_pps_nalu_bytes, byte[] send_general_nalu__start_bytes, byte[] send_general_nalu_bytes) {
        ...
        this.general_nalu_start_bytes = send_general_nalu__start_bytes;
        ...
        packet_info_bytes = new byte[header_bytes.length + sps_nalu_bytes.length + pps_nalu_bytes.length + general_nalu_start_bytes.length + general_nalu_bytes.length];
        ...
        if (general_nalu_start_bytes.length == 0) {
            ...
            System.arraycopy(general_nalu_start_bytes, 0, packet_info_bytes, header_bytes.length + sps_nalu_bytes.length + pps_nalu_bytes.length, general_nalu_start_bytes.length);
            System.arraycopy(general_nalu_bytes, 0, packet_info_bytes, header_bytes.length + sps_nalu_bytes.length + pps_nalu_bytes.length + general_nalu_start_bytes.length, general_nalu_bytes.length);
        }
    }
